...
puzzleInput = """Game 1: 12 blue, 15 red, 2 green; 17 red, 8 green, 5 blue; 8 red, 17 blue; 9 green, 1 blue, 4 red
...
def parseCubeSets(s) :
    return list(map(parseCubes, s.strip().split(",")))

def parseGames(s) :
    splitted = s.split(":")
    gameNumber = int(splitted[0].split(" ")[1])
    cubeSets = list(map(parseCubeSets, (splitted[1].split(";"))))
    return gameNumber , cubeSets

def isValidCube(c):
    if(color(c) == "red" and number(c) <= 12):
        return True
    if(color(c) == "green" and number(c) <= 13):
        return True
    if(color(c) == "blue" and number(c) <= 14):
        return True
    return False

def isValidCubeSet(cs):
    return all(map(isValidCube, cs))

def isGamePossible(g) :
    return all(map(isValidCubeSet, cubeSets(g)))
